...
def train(metadata, out_directory, verbose, weight_file, meta_file):
    ...
    model_features = ["response", "save_atari", "neighbor", "nakade_offset", "non_response_3x3_offset", "response_12d_offset"]
    model = policy.model
    # load weights
    if resume:
    ...
    for _layer in model.layers:
        if len( _layer.get_weights() ) > 1:
            print _layer.get_weights()[0].shape
            print _layer.get_weights()[1].shape
        print _layer.get_weights()
    '''
