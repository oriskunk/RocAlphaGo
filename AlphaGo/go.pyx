...
cdef class GameState:
    ...
    cdef void initialize_new(self, char size):
        ...
        # neighbor, neighbor3x3, neighbor12d, neighbor5x5 and zobrist_lookup are global
        self.neighbor       = neighbor
        self.neighbor3x3    = neighbor3x3
        self.neighbor12d    = neighbor12d
        self.neighbor5x5    = neighbor5x5
    ...
    cdef void initialize_duplicate(self, GameState copy_state):
        ...
        self.neighbor5x5     = copy_state.neighbor5x5
    ...
    def __init__(self, char size = 19, GameState copyState = None, enforce_superko=True):
        ...
        if copyState is not None:
            ...
            if not neighbor or size != neighbor_size:
                ...
                neighbor5x5    = get_5x5_neighbors(size)
    ...
    def __dealloc__(self):
        """
           ...
           !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           do not fee neighbor, neighbor3x3, neighbor12d, neighbor5x5,
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cdef complex128_t generate_nakade_hash(self, short centre):
        """
           generate nakade hash around centre location
        """
        cdef int    i
        #cdef long  hash = 5#_HASHVALUE
        cdef Group* group
        cdef short  x, y
        cdef short  colour, area_count = 0
        cdef complex128_t hash
        hash._0 = _HASHVALUE
        hash._1 = _HASHVALUE

        # calculate location in neighbor nakade array

        x = centre % self.size
        y = centre / self.size

        allow_colour = [ _BORDER ]
        allow_colour.append( _BLACK if self.player_current == _WHITE else _WHITE )

        # N
        colour = self.board_groups[ calculate_board_location_or_border(x, y - 1, self.size) ].colour
        #print 'N:', colour
        if colour == self.player_current:
            ...
            area_count += 1
            colour = self.board_groups[ calculate_board_location_or_border(x, y - 2, self.size) ].colour
            if colour == _EMPTY or colour == self.player_current:

                return hash

            # NW
            colour = self.board_groups[ calculate_board_location_or_border(x - 1, y - 1, self.size) ].colour
            if colour == self.player_current:
                ...
                area_count += 1
                if  self.board_groups[ calculate_board_location_or_border(x - 2, y - 1, self.size) ].colour not in allow_colour or \
                    self.board_groups[ calculate_board_location_or_border(x - 1, y - 2, self.size) ].colour not in allow_colour or \
                    self.board_groups[ calculate_board_location_or_border(x - 1, y    , self.size) ].colour is not _EMPTY:
        ...
        # W
        colour = self.board_groups[ calculate_board_location_or_border(x - 1, y, self.size) ].colour
        #print 'W:', colour
        if colour == self.player_current:
            ...
            area_count += 1
            colour = self.board_groups[ calculate_board_location_or_border(x - 2, y, self.size) ].colour
            if colour == _EMPTY or colour == self.player_current:

                return hash

            # NW
            if self.board_groups[ calculate_board_location_or_border(x, y - 1, self.size) ].colour is not _EMPTY:
                colour = self.board_groups[ calculate_board_location_or_border(x - 1, y - 1, self.size) ].colour
                if colour == self.player_current or colour == _EMPTY:
        ...
        #print area_count
        if area_count < 3 or area_count > 5:
        #if area_count > 5:
        ...
        for i in range(8):
            ...
            # hash colour
            hash._1 += group.colour
            hash._1 *= _HASHVALUE

            # hash liberty
            hash._1 += min( group.count_liberty, 3 )
            hash._1 *= _HASHVALUE
        ...
        for i in range(12):
            ...
            # get group
            group = self.board_groups[ self.neighbor5x5[ centre + i ] ]

            # hash colour
            hash._0 += group.colour
            hash._0 *= _HASHVALUE

            # hash liberty
            hash._0 += min( group.count_liberty, 3 )
            hash._0 *= _HASHVALUE

            # get group
            group = self.board_groups[ self.neighbor5x5[ centre + i + 12 ] ]
    ...
    cdef void get_group_after_pointer(self, short* stones, short* liberty, short* capture, char* locations, char* captures, short location):
        """
           ...
           liberty[0 ] to liberty count
           capture[0 ] to capture count
        """
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cdef complex128_t get_hash_nakade(self, short centre):
        """
           return 12d pattern hash + current player
        """

        # get nakade hash value and add current player colour

        hash = self.generate_nakade_hash(centre)

        hash._1 += 0 if hash._1 == _HASHVALUE else self.player_current
    ...
    cdef Locations_List* get_sensible_moves(self):
        """
           ...
           return a list with sensible legal moves
        """
    ...
    def _get_hash_3x3( self, action ):

        cdef short location
        ( x, y ) = action
        location = self.calculate_board_location( y, x )

        return self.get_hash_3x3( location )


    def _get_hash_12d( self, action, xDis, yDis ):

        cdef short location
        ( x, y ) = action
        location = self.calculate_board_location( y, x )

        hash_base = self.get_hash_12d( location )

        return mpz(hash_base + xDis) * _HASHVALUE + yDis


    def _get_hash_nakade( self, action ):

        cdef short location
        ( x, y ) = action
        location = self.calculate_board_location( y, x )

        _hash = self.get_hash_nakade( location )

        if _hash._1 == _HASHVALUE:

            return mpz(-1)

        hash = mpz(_hash._0)*59604644775390625 + _hash._1
        print _hash._0, _hash._1, hash
