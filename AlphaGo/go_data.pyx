...
cdef short* get_12d_neighbors(char size):
    ...
    for x in range(size):
        ...
        for y in range(size):
            ...
            '''
            neighbor12d[ location +  0 ] = calculate_board_location_or_border(x    , y - 2, size)

            neighbor12d[ location +  1 ] = calculate_board_location_or_border(x - 1, y - 1, size)
            neighbor12d[ location +  2 ] = calculate_board_location_or_border(x    , y - 1, size)
            neighbor12d[ location +  3 ] = calculate_board_location_or_border(x + 1, y - 1, size)

            neighbor12d[ location +  4 ] = calculate_board_location_or_border(x - 2, y    , size)
            neighbor12d[ location +  5 ] = calculate_board_location_or_border(x - 1, y    , size)
            neighbor12d[ location +  6 ] = calculate_board_location_or_border(x + 1, y    , size)
            neighbor12d[ location +  7 ] = calculate_board_location_or_border(x + 2, y    , size)

            neighbor12d[ location +  8 ] = calculate_board_location_or_border(x - 1, y + 1, size)
            neighbor12d[ location +  9 ] = calculate_board_location_or_border(x    , y + 1, size)
            neighbor12d[ location + 10 ] = calculate_board_location_or_border(x + 1, y + 1, size)

            neighbor12d[ location + 11 ] = calculate_board_location_or_border(x    , y + 2, size)
            '''
...
@cython.boundscheck( False )
@cython.wraparound(  False )
cdef short* get_5x5_neighbors( char size ):

    # create array
    cdef short* neighbor5x5 = <short *>malloc( size * size * 24 * sizeof( short ) )
    if not neighbor5x5:
        raise MemoryError()
    ...
    for x in range( size ):
        ...
        for y in range( size ):
            ...
            location = ( x + ( y * size ) ) * 24
            neighbor5x5[ location +  0 ] = calculate_board_location_or_border( x - 2, y - 2, size )
            neighbor5x5[ location +  1 ] = calculate_board_location_or_border( x - 1, y - 2, size )
            neighbor5x5[ location +  2 ] = calculate_board_location_or_border( x    , y - 2, size )
            neighbor5x5[ location +  3 ] = calculate_board_location_or_border( x + 1, y - 2, size )
            neighbor5x5[ location +  4 ] = calculate_board_location_or_border( x + 2, y - 2, size )

            neighbor5x5[ location +  5 ] = calculate_board_location_or_border( x - 2, y - 1, size )
            neighbor5x5[ location +  6 ] = calculate_board_location_or_border( x - 1, y - 1, size )
            neighbor5x5[ location +  7 ] = calculate_board_location_or_border( x    , y - 1, size )
            neighbor5x5[ location +  8 ] = calculate_board_location_or_border( x + 1, y - 1, size )
            neighbor5x5[ location +  9 ] = calculate_board_location_or_border( x + 2, y - 1, size )

            neighbor5x5[ location + 10 ] = calculate_board_location_or_border( x - 2, y    , size )
            neighbor5x5[ location + 11 ] = calculate_board_location_or_border( x - 1, y    , size )
            neighbor5x5[ location + 12 ] = calculate_board_location_or_border( x + 1, y    , size )
            neighbor5x5[ location + 13 ] = calculate_board_location_or_border( x + 2, y    , size )

            neighbor5x5[ location + 14 ] = calculate_board_location_or_border( x - 2, y + 1, size )
            neighbor5x5[ location + 15 ] = calculate_board_location_or_border( x - 1, y + 1, size )
            neighbor5x5[ location + 16 ] = calculate_board_location_or_border( x    , y + 1, size )
            neighbor5x5[ location + 17 ] = calculate_board_location_or_border( x + 1, y + 1, size )
            neighbor5x5[ location + 18 ] = calculate_board_location_or_border( x + 2, y + 1, size )

            neighbor5x5[ location + 19 ] = calculate_board_location_or_border( x - 2, y + 2, size )
            neighbor5x5[ location + 20 ] = calculate_board_location_or_border( x - 1, y + 2, size )
            neighbor5x5[ location + 21 ] = calculate_board_location_or_border( x    , y + 2, size )
            neighbor5x5[ location + 22 ] = calculate_board_location_or_border( x + 1, y + 2, size )
            neighbor5x5[ location + 23 ] = calculate_board_location_or_border( x + 2, y + 2, size )
