...
cdef class Preprocess:
    ...
    cdef int get_response(self, GameState state, tensor_type[ :, ::1 ] tensor, int offSet):
        ...
        if state.moves_history.count > 0 and last_move != _PASS:
            ...
            for i in range(12):
                ...
                if state.board_groups[ location ].colour == _EMPTY:
                    ...
                    # dictionary lookup
                    id = self.pattern_response_12d.get( hash_pattern, -1 )
    ...
    cdef int get_nakade(self, GameState state, tensor_type[ :, ::1 ] tensor, int offSet):
        ...
        for i in range(state.moves_legal.count):
            ...
            _hash = state.get_hash_nakade( location )

            if _hash._1 != _HASHVALUE:

                weight = self.pattern_nakade_w.get( mpz(_hash._0)*59604644775390625 + _hash._1, 0.0 )

                tensor[ offSet, location ] = weight

            #else:

            #    tensor[ offSet, location ] = 0.0#-3.3485961
    ...
    cdef int get_nakade_offset(self, GameState state, tensor_type[ :, ::1 ] tensor, int offSet):
        ...
        for i in range(state.moves_legal.count):
            ...
            if _hash._1 != _HASHVALUE:            
                ...
                id = self.pattern_nakade.get( mpz(_hash._0)*59604644775390625 + _hash._1, -1 )
        ...
        if state.moves_history.count > 0 and last_move != _PASS:
            ...
            for i in range(12):
                ...
                if state.board_groups[ location ].colour == _EMPTY:
                    ...
                    # dictionary lookup
                    weight = self.pattern_response_12d_w.get( hash_pattern, 0.0 )
    ...
    cdef int get_non_response_3x3(self, GameState state, tensor_type[ :, ::1 ] tensor, int offSet):
        ...
        for i in range(state.moves_legal.count):
            ...
            # get location hash and dict lookup
            weight = self.pattern_non_response_3x3_w.get( state.get_hash_3x3( location ), 0.0 )
    ...
    cdef int get_non_response_3x3_offset(self, GameState state, tensor_type[ :, ::1 ] tensor, int offSet):
        ...
        for i in range(state.moves_legal.count):
            ...
            location = state.moves_legal.locations[ i ]
            # get location hash and dict lookup
            id = self.pattern_non_response_3x3.get( state.get_hash_3x3( location ), -1 )
    ...
    def __init__(self, list feature_list, char size=19, dict_nakade='patterns/nakade.npy', dict_3x3='patterns/3x3.npy', dict_12d='patterns/12d.npy', dict_nakade_w='patterns/nakade_w.npy', dict_3x3_w='patterns/3x3_w.npy', dict_12d_w='patterns/12d_w.npy', verbose=True):
        """
        """
        ...
        if dict_nakade is not None:
            self.pattern_nakade = np.load(dict_nakade).item()
        ...
        if dict_12d is not None:
            self.pattern_response_12d = np.load(dict_12d).item()
        ...
        if dict_3x3 is not None:
            self.pattern_non_response_3x3 = np.load(dict_3x3).item()
        ...
        # load nakade_w patterns
        self.pattern_nakade_w = {}
        if dict_nakade_w is not None:
            self.pattern_nakade_w = np.load(dict_nakade_w).item()

        # load 12d_w response patterns
        self.pattern_response_12d_w = {}
        if dict_12d_w is not None:
            self.pattern_response_12d_w = np.load(dict_12d_w).item()

        # load 3x3_w non response patterns
        self.pattern_non_response_3x3_w = {}
        if dict_3x3_w is not None:
            self.pattern_non_response_3x3_w = np.load(dict_3x3_w).item()
        ...
        for i in range(len(feature_list)):
            ...
            #elif feat == "neighbor_offset":
            #    processor            = self.get_neighbor_offset
            #    self.output_dim     += 8

            elif feat == "nakade_offset":
                processor            = self.get_nakade_offset
                self.output_dim     += self.pattern_nakade_size

            elif feat == "response_12d_offset":
                processor            = self.get_response_12d_offset
                self.output_dim     += self.pattern_response_12d_size

            elif feat == "non_response_3x3_offset":
                processor            = self.get_non_response_3x3_offset
    ...
    cdef np.ndarray[ tensor_type, ndim=4 ] generate_tensor(self, GameState state):
        ...
        cdef np.ndarray[ tensor_type, ndim=2 ] np_tensor = np.zeros((self.output_dim, self.board_size), dtype=np.float32)
